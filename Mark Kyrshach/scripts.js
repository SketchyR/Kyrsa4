...
function createClassChart(statistics) {
   ...
   const colors = ['#FF5733', '#33FF57', '#5733FF', '#33FFAA', '#FF33DD']; // Массив с предопределенными цветами для каждого предмета
...
subjects.forEach((subject, index) => {
    const dataset = {
        label: subject,
        data: [],
        backgroundColor: colors[index], // Используем предопределенные цвета
        borderWidth: 1
    };
